...
const CommentList = [
  {
    name: "abc",
    text: "lorem uhdosdoish sipdhiosjpojsd pojuds",
    replies: [],
  },
  {
    name: "abc",
...
const Comment = ({ comments }) => {
  ...
  return (
    <div className="flex m-1 p-2 bg-gray-100">
      <img
      ...
      <div className="">
        <h1>{name}</h1>
        <span className="font-normal text-m"> {text}</span>
      </div>
    </div>
  );
};
  ...
  return (
    <div>
      {commentList.map((c, index) => (
        <div>
          <Comment comments={c} key={index} />
          <div className="ml-5 border border-l-black-500 ">
            <Comments commentList={c.replies} />
          </div>
        </div>
      ))}
    </div>
